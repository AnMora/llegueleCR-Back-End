...
module.exports.EnterpriseController = {
  getEnterprises: async (req, res) => {
    try {
      ...
      Response.success(res, 200, "Enterprises List", Enterprises);
    } catch (err) {
      debug(err);
      Response.fail(res);
    }
  ...
  getEnterprise: async (req, res) => {
    try {
      ...
      if (!Enterprise) {
        ...
        Response.success(res, 200, `Enterprise ${id}`, Enterprise);
      }
    } catch (err) {
      debug(err);
      Response.fail(res);
    ...
    try {
      ...
      if (EnterpriseExists) {
        ...
        if (
          ...
          Response.success(res, 201, "Enterprise created", body);
        }
      }
    } catch (err) {
      debug(err);
    ...
    try {
      ...
      if (!Enterprise) {
        ...
        if (
          ...
          Response.success(res, 202, `Enterprise ${id} updated`, body);
        }
      }
    } catch (err) {
      debug(err);
